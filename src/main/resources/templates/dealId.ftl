...
<@c.page>
    ...
    <div class="collapse <#if !feedbacks??>show</#if>" id="collapseExample">
        <div class="form-group mt-3">
            <form method="post" action="/deal/newFeedback/${deal.id}" enctype="multipart/form-data">
                <label for="periodStr">Срок займа </label>
                <b><u>
                        <output for="periodStr" id="period_value">${deal.period}</output>
                    </u></b>
                <label for="periodStr"> дней.</label>
                <input type="range" class="custom-range" min="1" max="100"
                       value="${deal.period}" id="periodStr" name="period"
                       oninput="periodUpdate(value)">
                <br/><br/>
                <label for="amountStr">Сумма </label>
                <b><u>
                        <output for="amountStr" id="amount_value">${deal.amount}</output>
                    </u></b>
                <label for="amountStr"> рублей.</label>
                <input type="range" class="custom-range" min="10" max="3000"
                       value="${deal.amount}" id="amountStr" name="amount"
                       oninput="amountUpdate(value)">
                <br/><br/>
                <label for="percentStr">Оплата за пользование не более </label>
                <b><u>
                        <output for="percentStr" id="percent_value">${deal.percent}</output>
                    </u></b>
                <label for="percentStr"> % в 30 дней.</label>
                <input type="range" class="custom-range" min="1" max="100"
                       value="${deal.percent}" step="0.1" id="percentStr"
                       name="percent"
